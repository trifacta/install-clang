...
# Above steps install llvm & clang from packages, linking against libstdc++.
# We now need to create llvm & clang installs that aren't built with libstdc++.

ENV PARALLEL=20
ENV LLVM_INSTALL_PREFIX=/opt/llvm36
ENV CC /usr/bin/clang
ENV CXX  /usr/bin/clang++
ENV CXXFLAGS -stdlib=libc++ -Doffsetof=__builtin_offsetof
ENV LDFLAGS -stdlib=libc++  -lc++ -lc++abi -lrt -ldl -lutil
...
RUN /opt/install-clang/ubuntu-install-clang.sh
